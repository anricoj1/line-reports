class Order:
    def __init__(self, result, order, orderlines):
        self.result = result
        self.order = order
    ...
    def getOrder(self) -> dict:
        return {
            "User": self.result,
            "Products": [self.getSizes(x['Product']['Name'], x['Options'], x['Count'], x) for x in self.orderlines],
    ...
    def getSizes(self, product, options, count, orderlines):
            ...
            if int(count) == 1:
                if len(options) > 0:
                    if "Size" in options[0]['Name']:
                        ...
                        return [product[0:len(product) - 2] + options[0]['Value'], [x['Value'] for x in options[1:]]]
                    elif "Boar's Head" in options[0]['Name']:
                        return [product[0:len(product) - 2] + options[0]['Value'], [x['Value'] for x in options[1:]]]
                    else:
